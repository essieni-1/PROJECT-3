...
#define WIPERS_MINIMUM              (245) //0 degrees
...
void app_main(void) {
    ...
    while(true) {
        ...
        //update wiper mode
        wipe_off = mode_select <= OFF_THRESHOLD && mode_select > INT_THRESHOLD;
        wipe_int = mode_select <= INT_THRESHOLD && mode_select > LOW_THRESHOLD && ignition;
        ...
        if (ignition_button && !ignition_button_prev) {
            ...
            //if the conditions for the engine starting are not met
            else {      
...
void wiper_control_task(void *pvParameters) { 
    while(1) {
        if(wipe_high) {
            ...
            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, WIPERS_MINIMUM);
...
void perform_wipe_cycle(int steps) {
    ...
    int angle = WIPERS_MINIMUM;
    int interval = (WIPERS_MAXIMUM - WIPERS_MINIMUM) / steps;
    while(angle < WIPERS_MAXIMUM) {
